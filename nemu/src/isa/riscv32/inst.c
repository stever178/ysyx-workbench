...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  Log("inst = %08x, rs2 = %d, rs1 = %d, rd = %d", i, rs2, rs1, *rd);
  switch (type) {
  ...
  Log("    src1 = %08x, src2 = %08x, imm = %08x", *src1, *src2, *imm);
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  /* *** RV32I Base Instruction Set *** */
  {
    ...
    INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (src1 >> src2) | (~0U << (32 - src2)));
    ...
    INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (src1 >> imm) | ((src1 & 0x80000000) << (32 - imm)));
  ...
  /* *** RV32M Standard Extension *** */
  {
    ...
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (uint64_t)(((int64_t)src1 * (int64_t)src2) & MASK_HIGH_32_OF_64) >> 32);
    INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulu   , R, R(rd) = (((uint64_t)src1 * (uint64_t)src2) & MASK_HIGH_32_OF_64) >> 32);

...
#define MASK_HIGH_32_OF_64 (0xffffffff00000000)
...
#define immI()                                                                 \
  do {                                                                         \
    if (BITS(i, 31, 26) == 0b010000 && BITS(i, 14, 12) == 0b0101) {            \
      *imm = SEXT(BITS(i, 25, 20), 6);                                         \
    } else {                                                                   \
...
#define immJ()                                                                 \
  do {                                                                         \
    word_t _imm = (BITS(i, 31, 31) << 20 | /* bit 20 */                        \
                   BITS(i, 30, 21) << 1 |  /* bits 10:1 */                     \
                   BITS(i, 20, 20) << 11 | /* bit 11 */                        \
                   BITS(i, 19, 12) << 12   /* bits 19:12 */                    \
    );                                                                         \
    *imm = SEXT(_imm, 21);                                                     \
  } while (0)
...
#define immB()                                                                 \
  do {                                                                         \
    word_t _imm = (BITS(i, 31, 31) << 12 | /* bit 12 */                        \
                   BITS(i, 30, 25) << 5 |  /* bits 10:5 */                     \
                   BITS(i, 11, 8) << 1 |   /* bits 4:1 */                      \
                   BITS(i, 7, 7) << 11     /* bit 11 */                        \
    );                                                                         \
    *imm = SEXT(_imm, 13);                                                     \
  } while (0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  ...
  // Log("inst = %08x, rd = %d, rs1 = %d, rs2 = %d", i, *rd, rs1, rs2);
  // Log("        imm = %08x, src1 = %08x, src2 = %08x", *imm, *src1, *src2);
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  /* *** RV32I Base Instruction Set *** */
  {
    ...
    INSTPAT("0000??? ????? 00000 000 00000 00011 11", fence  , I, /* todo */
  ...
  /* *** RV32M Standard Extension *** */
  {
    ...
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (((int64_t)(int32_t)src1 * (int64_t)(int32_t)src2) & MASK_HIGH_32_OF_64) >> 32);
    INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (((int64_t)(int32_t)src1 * (uint64_t)src2) & MASK_HIGH_32_OF_64) >> 32);
    INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (((uint64_t)src1 * (uint64_t)src2) & MASK_HIGH_32_OF_64) >> 32);
    INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R,
        if (src2 == 0) { R(rd) = -1; } else {
          sword_t a = (sword_t)src1;
          sword_t b = (sword_t)src2;
          if ((word_t)a == (1ull << 31) && b == -1) {
            R(rd) = a;
          } else {
            R(rd) = a / b;
          }
